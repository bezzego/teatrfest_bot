...
async def cmd_start(message: Message, state: FSMContext, db: Database, config: Config):
    ...
    if slug and mapping:
        ...
        formatted_datetime = format_datetime_readable(mapping['show_datetime']) if mapping.get('show_datetime') else ''
        project_info = (
        ...
        from utils.utils import format_datetime_readable
        formatted_datetime = format_datetime_readable(show_datetime)
        project_info = (
            f"\n\n📋 Информация о вашем спектакле:\n"
            f"🏙️ Город: {city}\n"
    ...
    welcome_text = (
        "Здравствуйте! 👋\n\n"
        f"<b>Вы на странице спектакля «{project_name}» "
        "от компании «Театральный Фестиваль».</b>\n"
        "<i>🎁 Здесь вы можете получить персональную скидку на билеты "
        "и помощь по любым вопросам, связанным с посещением спектакля.</i>\n\n"
        "<b>НО… В театре есть одна важная деталь 🎭</b>\n"
